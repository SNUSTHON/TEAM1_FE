  ...
  return (
    <>
      <div className="profile-section">
        <Image src={defaultAvatarSvg} alt="Profile" width={60} height={60} className="profile-image" />
        <div className="profile-info">
          <h2>반갑습니다 {username}님</h2>
          <p>오늘도 생각을 넓혀볼까요?</p>
        </div>
        <button className="create-memo-btn">
          + New Idea
        </button>
      </div>
      ...
      <div className="canvas-grid">
        {canvases.map((canvas) => (
          <div key={canvas.id} className="canvas-item">
            <Image src={defaultCanvasSvg} alt={canvas.subject} width={300} height={200} className="canvas-image" />
            <div className="canvas-info">
              <h3 className="canvas-title">{canvas.subject}</h3>
              <p className="canvas-date">Edited {new Date(canvas.updatedAt).toLocaleString()}</p>
            </div>
          </div>
        ))}
        {[...Array(3)].map((_, index) => (
          <div key={`new-${index}`} className="canvas-item new-canvas" onClick={handleCreateCanvas}>
